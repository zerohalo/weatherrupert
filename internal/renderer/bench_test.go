...
func BenchmarkSlideCurrentConditions(b *testing.B) {
	data := mockData()
	benchSlide(b, NewSlideCurrentConditions(false, false, nil, nil, nil, defaultFonts), data)
}

func BenchmarkSlideHourlyForecast(b *testing.B) {
	data := mockData()
	benchSlide(b, NewSlideHourlyForecast(false, false, nil, nil, nil, defaultFonts), data)
}

func BenchmarkSlidePrecipitation(b *testing.B) {
	data := mockData()
	benchSlide(b, NewSlidePrecipitation(false, false, nil, nil, nil, defaultFonts), data)
}

func BenchmarkSlideExtendedForecast(b *testing.B) {
	data := mockData()
	benchSlide(b, NewSlideExtendedForecast(false, false, nil, nil, nil, defaultFonts), data)
}
