...
func New(w, h, frameRate int, label string,
	...
	return &Renderer{
		...
		fonts:            fonts,
		weatherSlides: []weatherSlideEntry{
			{name: "alerts", fn: NewSlideAlerts(use24h, fonts), skip: func(d *weather.WeatherData) bool { return len(d.Alerts) == 0 }},
			...
			{name: "solar-weather", fn: NewSlideSolarWeather(use24h, useMetric, fonts), skip: func(d *weather.WeatherData) bool {
		...
		specialSlides: []specialSlideEntry{
			{
				name:        "announcements",
				fn:          NewSlideAnnouncements(getAnnouncements, getAnnDuration, use24h, fonts),
			...
			{name: "trivia", fn: NewSlideTrivia(getTriviaItems, getTriviaDuration, getTriviaRandomize, use24h, fonts), getInterval: getTriviaInterval},
		},
