...
I really loved the idea behind ws4channels serving WS4KP but wanted to push it further: drop the headless browser entirely, render everything procedurally in Go, and only do work when someone is actually watching. The goal is that the service does essentially nothing when it's not being used — no CPU, no network. When no viewers are connected, weather and solar data fetches pause, music stream connections are dropped, and the video encoder is killed. Everything starts fresh the moment a viewer tunes in — video with silence appears immediately, then music kicks in within a second or two. The result is a lightweight stream that serves both MPEG-TS and HLS to fit different home setups, and doubles as a family information board — weather, trivia, scrolling announcements, and background music all on one channel.
